...
export async function sendEmailChangeVerification(input: {
  ...
  const expiresLabel = `${VERIFICATION_TTL_MINUTES} minutes`;
  const subject = "Confirm your new EternalNotes email address";
  const text = [
    `Hi ${input.name || "there"},`,
    "",
    `Your EternalNotes email-change verification code is: ${input.code}`,
    "",
    `This code expires in ${expiresLabel}.`,
    "",
    `If you did not request this, you can safely ignore this email — your address will not be changed.`,
    "",
    `Sent ${now()}`
  ].join("\n");
  ...
  const html = `
    <div style="font-family: Inter, Arial, sans-serif; background:#0e1116; color:#f4f7fb; padding:24px;">
      <div style="max-width:560px; margin:0 auto; border:1px solid #2a3342; background:#141922; border-radius:16px; padding:24px;">
        <div style="font-size:12px; letter-spacing:0.14em; text-transform:uppercase; color:#a78bfa; font-weight:700;">EternalNotes</div>
        <h1 style="margin:12px 0 8px; font-size:24px; line-height:1.2;">Confirm your new email</h1>
        <p style="margin:0 0 20px; color:#aab4c3; line-height:1.6;">Enter this code in EternalNotes to confirm <strong style="color:#f4f7fb;">${input.email}</strong> as your new address. The code expires in ${expiresLabel}.</p>
        <div style="font-size:32px; font-weight:700; letter-spacing:0.18em; padding:16px 20px; border-radius:12px; background:#191f2a; border:1px solid #2a3342; text-align:center;">${input.code}</div>
        <p style="margin:20px 0 0; color:#748094; line-height:1.6;">If you did not request this change, your current email address will remain unchanged.</p>
      </div>
    </div>
  `;
  ...
  if (resendKey && emailFrom) {
    ...
    if (!response.ok) {
      const payload = await response.text();
      throw new Error(`Email change verification failed: ${payload || response.statusText}`);
    }
    return { debugCode: null };
  }

  const devMode = (process.env.EMAIL_VERIFICATION_DEV_MODE ?? (process.env.NODE_ENV !== "production" ? "true" : "false")).toLowerCase() === "true";
  if (devMode) return { debugCode: input.code };
