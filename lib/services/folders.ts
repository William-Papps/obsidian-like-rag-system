...
export async function listFolders(userId: string): Promise<Folder[]> {
  const rows = await dbAll("select * from folders where user_id = ? and workspace_id is null order by name collate nocase", [userId]);
...
export async function createFolder(userId: string, name: string, parentId: string | null = null): Promise<Folder> {
  ...
  await dbRun("insert into folders (id, user_id, parent_id, workspace_id, name, created_at, updated_at) values (?, ?, ?, null, ?, ?, ?)", [
    folder.id,
    userId,
    parentId,
    folder.name,
